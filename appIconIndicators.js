...
export class AppIconIndicator {
    constructor(source) {
        ...
        case RunningIndicatorStyle.DEFAULT:
            runningIndicator = new RunningIndicatorUnderline(source);
            break;
        ...
        case RunningIndicatorStyle.TIDE:
            runningIndicator = new RunningIndicatorUnderline(source);
            break;
...
// Full-height canvas so underline pills can sit below the icon, not on it.
const UnderlineDrawingArea = GObject.registerClass(
class UnderlineDrawingArea extends St.DrawingArea {
    vfunc_allocate(box) {
        if (box.x1 !== 0 || box.y1 !== 0)
            return super.vfunc_allocate(box);

        this.set_allocation(box);
        return St.DrawingArea.prototype.vfunc_allocate.call(this, box);
    }
});
...
class RunningIndicatorUnderline extends RunningIndicatorDots {
    constructor(source) {
        super(source);

        // RunningIndicatorDots uses a square canvas sized to the icon, which
        // forces the pill onto the icon art. Use a full-height canvas instead.
        this._area.destroy();
        this._area = new UnderlineDrawingArea({
            x_expand: true,
            y_expand: true,
        });
        this._area.set_pivot_point(0.5, 0.5);
        ...
        if (this._side === St.Side.BOTTOM) {
            const icon = this._source._iconContainer.get_children()[0];
            if (icon)
                icon.set_style(`margin-bottom: ${this._pillGap}px;`);
        }
    }
    ...
    _computeStyle() {
        ...
        this._padding = 2;
        this._borderWidth = 0;
        // Pill spans ~65% of icon width, centered underneath.
        this._lengthScale = 0.65;

        const Color = Clutter.Color ?? Cogl.Color;
        const alpha = this._source.focused ? 230 : 115;
        this._bodyColor = new Color({red: 255, green: 255, blue: 255, alpha});
    }
    ...
    _drawIndicator(cr) {
        ...
        const segments = this._number;
        const barHeight = Math.max(4, Math.round(this._width / 8));
        const gap = segments > 1 ? 3 : 0;
        const pillWidth = this._width * this._lengthScale;
        const xInset = (this._width - pillWidth) / 2;
        const segmentWidth = (pillWidth - gap * (segments - 1)) / segments;
        const radius = Math.min(barHeight / 2, segmentWidth / 2);
        const yOffset = this._side === St.Side.BOTTOM
            ? this._height - this._padding - barHeight
            : this._padding;

        Utils.cairoSetSourceColor(cr, this._bodyColor);
        cr.translate(xInset, yOffset);
        for (let i = 0; i < segments; i++) {
            const x = i * (segmentWidth + gap);
            cr.newSubPath();
            cr.moveTo(x + segmentWidth - radius, 0);
            cr.lineTo(x + radius, 0);
            cr.arcNegative(x + radius, radius, radius, -Math.PI / 2, Math.PI / 2);
            cr.lineTo(x + segmentWidth - radius, barHeight);
            cr.arcNegative(x + segmentWidth - radius, radius, radius, Math.PI / 2, -Math.PI / 2);
            cr.closePath();
        }
        cr.fill();
    }
