...
import {THE_PILL_DEFAULTS} from './defaults.js';
...
const TIDE_SHOW_ANIMATION = 0.18;
const TIDE_HIDE_ANIMATION = 0.18;

export {THE_PILL_DEFAULTS};
...
const DashSlideContainer = GObject.registerClass({
    ...
    vfunc_allocate(box) {
        ...
        if (this.side === St.Side.LEFT) {
            ...
            childBox.y1 = (this.slideX - 1) * (childHeight - slideoutSize);
            childBox.y2 = slideoutSize + this.slideX * (childHeight - slideoutSize);
...
const DockedDash = GObject.registerClass({
    ...
    _init(params) {
        ...
        super._init({
            ...
            reactive: false,
            style_class: `${Theming.PositionStyleClass[this._position]} minimal`,
        });
        ...
        this.staticBox = {x1: 0, y1: 0, x2: 0, y2: 0};
    ...
    _updateDashMaxSize() {
        ...
        if (this._isHorizontal) {
            this.dash.maxWidth = Math.round(fraction * workArea.width);
            this.dash.maxHeight = -1;
        } else {
            this.dash.maxHeight = Math.round(fraction * workArea.height);
            this.dash.maxWidth = -1;
        }
    }

    _initialize() {
        if (DockManager.settings.extendHeight) {
            if (this._isHorizontal) {
    ...
    _onDestroy() {
        ...
        Main.wm._workspaceSwitcherPopup?.disconnectObject(this);
    ...
    _updateDashVisibility() {
        ...
        if (DockManager.settings.dockFixed) {
            ...
            if (this.dash.requiresVisibility ||
                (this._autohideIsEnabled && this._box.hover))
                this._animateIn(settings.animationTime, 0);
            else
    ...
    _animateIn(time, delay) {
        ...
        const duration = time > 0 ? TIDE_SHOW_ANIMATION : 0;
    ...
    _animateOut(time, delay) {
        ...
        const duration = time > 0 ? TIDE_HIDE_ANIMATION : 0;
    ...
    _isPointerNearDockEdge(x, y) {
        const workArea = Main.layoutManager.getWorkAreaForMonitor(this._monitor.index);
        const tolerance = 12;

        switch (this._position) {
        case St.Side.LEFT:
            return x >= this._monitor.x && x <= this._monitor.x + tolerance &&
                y >= workArea.y && y <= workArea.y + workArea.height - 1;
        case St.Side.RIGHT: {
            const edgeX = this._monitor.x + this._monitor.width - 1;
            return x >= edgeX - tolerance && x <= edgeX &&
                y >= workArea.y && y <= workArea.y + workArea.height - 1;
        }
        case St.Side.TOP:
            return y >= this._monitor.y && y <= this._monitor.y + tolerance &&
                x >= workArea.x && x <= workArea.x + workArea.width - 1;
        case St.Side.BOTTOM: {
            const monitorEdgeY = this._monitor.y + this._monitor.height - 1;
            const workAreaEdgeY = workArea.y + workArea.height - 1;
            const edgeY = Math.min(monitorEdgeY, workAreaEdgeY);
            return y >= edgeY - tolerance && y <= edgeY + tolerance &&
                x >= workArea.x && x <= workArea.x + workArea.width - 1;
        }
        default:
            return false;
    ...
    _checkDockDwell(x, y) {
        const shouldDwell = this._isPointerNearDockEdge(x, y);
    ...
    _updatePressureBarrier() {
        ...
        if (this._canUsePressure && this._autohideIsEnabled &&
            ...
            this._pressureBarrier.connectObject('trigger', _barrier => {
    ...
    _centerOnMonitor() {
        ...
        if (this._isHorizontal) {
            if (this.width <= 0)
                return;
            this.x = workArea.x + Math.round((workArea.width - this.width) / 2);
        } else {
            if (this.height <= 0)
                return;
            this.y = workArea.y + Math.round((workArea.height - this.height) / 2);
        }
    }
    ...
    _updateStaticBox() {
        ...
        this.staticBox.x1 = x;
        this.staticBox.y1 = y;
        this.staticBox.x2 = x + this._box.width;
        this.staticBox.y2 = y + this._box.height;
...
export class DockManager {
    ...
    // Trash/mount icons are locked off for The Pill; only tear down any leftovers.
    _ensureLocations() {
        if (this._fm1Client) {
    ...
    _applyThePillDefaults() {
        for (const [key, value] of Object.entries(THE_PILL_DEFAULTS)) {
    ...
    _prepareMainDash() {
        ...
        this._vfuncInjections.addWithLabel(Labels.MAIN_DASH, ControlsManagerLayout.prototype,
            'allocate', function (originalAllocate, container) {
                ...
                originalAllocate.call(this, container, workAreaBox);
        ...
        this._vfuncInjections.addWithLabel(Labels.MAIN_DASH, Workspace.WorkspaceBackground.prototype,
            'allocate', function (originalAllocate, box) {
                /* eslint-disable no-invalid-this */
