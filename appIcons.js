...
export const DockAbstractAppIcon = GObject.registerClass({
    ...
    ownsWindow(window) {
        return this.app === getWindowTracker().get_window_app(window);
    }
    ...
    _updateFocusState() {
        this.focused = getWindowTracker().focus_app === this.app && this.running;
    }
    ...
    _toggleMinimizeMaximize() {
        const windows = this.getInterestingWindows();
        if (windows.length === 0)
            return;

        if (this.focused) {
            this._minimizeWindow(windows.length > 1);
        } else {
    ...
    _getWindowToClose() {
        return selectWindowToClose({
            windows: this.getInterestingWindows(),
            focusWindow: global.display.focus_window,
            ownsWindow: w => this.ownsWindow(w),
            workspaceWindows: global.workspace_manager
                .get_active_workspace().list_windows(),
        });
    }

    // Close one window: the focused one when present, otherwise the topmost
    // on the current workspace (or the only window when just one is open).
    closeFocusedWindow() {
        const window = this._getWindowToClose();
        if (window)
...
const DockAppIcon = GObject.registerClass({
    ...
    _init(app, monitorIndex, iconAnimator) {
        ...
        this._signalsHandler.add(getWindowTracker(), 'notify::focus-app', () => this._updateFocusState());
    }
});
...
class DockShowAppsIconMenu extends DockAppIconMenu {
    _rebuildMenu() {
        this.removeAll();
        // No preferences UI ships with The Pill; keep an empty menu.
    }
}
