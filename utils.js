...
const {InjectionManager} = Extension;
...
/**
 * Manage vfunction injection via InjectionManager (no direct imports._gi).
 */
export class VFuncInjectionsHandler extends BasicHandler {
    ...
    _create(prototype, name, injectedFunction) {
        const methodName = name.startsWith('vfunc_') ? name : `vfunc_${name}`;
        const original = prototype[methodName];
        if (!(original instanceof Function))
            throw new Error(`Virtual function ${name} is not available for ${prototype}`);

        this._injectionManager.overrideMethod(prototype, methodName,
            originalMethod => {
                return function (...args) {
                    return injectedFunction.call(this, originalMethod, ...args);
                };
            });
    ...
    _remove(item) {
        const [prototype, methodName] = item;
