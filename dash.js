...
const ICON_HOVER_SCALE = 1.12;
const ICON_HOVER_LIFT = 4;
const ICON_HOVER_DURATION = 150;
...
export const DockDash = GObject.registerClass({
    ...
    _init(monitorIndex) {
        ...
        this._showAppsIcon.set_pivot_point(0.5, 0.5);
        this.showAppsButton.connect('notify::hover', () => {
            this._applyIconHoverEffect(this._showAppsIcon, this.showAppsButton.hover);
            if (this._showAppsIcon.get_parent() === this._boxContainer)
                this._ensureItemVisibility(this.showAppsButton);
        });
    ...
    _createAppItem(app) {
        ...
        appIcon.set_pivot_point(0.5, 0.5);
        appIcon.connectObject('notify::hover', a => {
            this._applyIconHoverEffect(a, a.hover);
    ...
    _getIconHoverTranslation(hovered) {
        if (!hovered)
            return {x: 0, y: 0};

        switch (this._position) {
        case St.Side.TOP:
            return {x: 0, y: ICON_HOVER_LIFT};
        case St.Side.LEFT:
            return {x: ICON_HOVER_LIFT, y: 0};
        case St.Side.RIGHT:
            return {x: -ICON_HOVER_LIFT, y: 0};
        default:
            return {x: 0, y: -ICON_HOVER_LIFT};
    ...
    _applyIconHoverEffect(actor, hovered) {
        if (!actor?.get_stage())
            return;

        const {x, y} = this._getIconHoverTranslation(hovered);
        const scale = hovered ? ICON_HOVER_SCALE : 1;
    ...
    _redisplay() {
        ...
        if (settings.alwaysCenterIcons) {
            if (this._isHorizontal)
                this._scrollView.xAlign = Clutter.ActorAlign.CENTER;
            else
                this._scrollView.yAlign = Clutter.ActorAlign.CENTER;
        } else if (dockManager.settings.dockExtended) {
            if (!this._isHorizontal)
                this._scrollView.yAlign = Clutter.ActorAlign.START;
            else
                this._scrollView.xAlign = Clutter.ActorAlign.START;
        }
